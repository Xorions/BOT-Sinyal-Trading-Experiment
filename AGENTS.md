...
- **ccxt** untuk eksekusi order di Bybit Futures (USDT Perpetual, linear) — hanya dibutuhkan saat autotrade aktif
...
- Sinyal BUY/SELL **dipromosikan hanya jika ≥ `CONFLUENCE_MIN` item cek selaras** (default 2) **DAN** minimal `REQUIRE_CONFLUENCE_CORE` cek dari kategori inti (**S&D/S&R atau MACD/RSI**) selaras; selain itu diturunkan ke NEUTRAL (watchlist) dengan catatan alasan. SMC/OB + Whale/Vol saja **tidak cukup** — mencegah sinyal bermodal konfluensi sempit (mis. SMC/OB 2/2 tapi S&D/S&R 0/2 & MACD/RSI 0/2). Karena bot ini dapat **mengeksekusi order nyata di Bybit Futures**, filter ini adalah pengaman utama terhadap sinyal lemah.
...
|---|---|---|
| `BYBIT_API_KEY` | kosong | API Key Bybit (https://www.bybit.com/app/user/api-management, UTA, izin Order) |
| `BYBIT_SECRET_KEY` | kosong | Secret Key Bybit — API v5 TIDAK memakai passphrase |
| `ENABLE_BYBIT_AUTOTRADE` | `false` | Sakelar utama autotrade; `true` = order NYATA |
...
1. `build_exchange()` → `ccxt.bybit` (options `defaultType='linear'` untuk USDT Perpetual) + `load_markets()`. Raise `BybitExecutionError` bila kredensial kosong.
2. `market_symbol("BTC")` → `BTC/USDT:USDT` (USDT Perpetual, linear).
3. `fetch_free_balance_usdt()` → free USDT akun linear; raise bila 0.
4. **Position sizing dinamis**: `risk = balance × RISK_PERCENT_PER_TRADE / 100`; koin = `risk / |entry − SL|` (arah LONG/SHORT sama). Nilai posisi dibatasi ≤ saldo (margin 1x) agar tidak ditolak; koin dikonversi ke kontrak (`contractSize` Bybit < 1 koin, mis. BTCUSDT = 0.001).
5. **Market order entry** dengan `stopLossPrice` terpasang di order — ccxt.bybit mengubahnya menjadi field `stopLoss` (harga trigger) pada create-order v5, SL full position.
...
### Kontrak tes (tests/test_bybit_executor.py)
...
### Mengubah perilaku eksekusi Bybit
- Logika order ada di `execution/bybit_executor.py` → `execute_signal`. Sizing: `position_quantity` + `risk_amount_usd`. SL terpasang via params `stopLossPrice` (ccxt → field `stopLoss` order Bybit v5); TP pakai `reduceOnly: true`.
...
- Untuk Bybit: gunakan API key Unified Trading Account dengan izin minimal **Order/Trade** (jangan pernah aktifkan Withdraw). API v5 tidak memakai passphrase — cukup apiKey + secret. Test di akun demo/posisi kecil sebelum nyalakan `ENABLE_BYBIT_AUTOTRADE=true`.
