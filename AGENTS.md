...
**Opsional — Automated Trading OKX (modul terpisah, default NONAKTIF)**: bila `ENABLE_OKX_AUTOTRADE=true`, sinyal BUY/SELL yang lolos filter dieksekusi oleh `execution/okx_executor.py` (market order + SL + TP1 50%/TP2 50% di OKX Futures USDT-M via CCXT), dengan laporan live 🚀/⚠️ ke `TELEGRAM_ADMIN_CHAT_ID`. Modul ini sepenuhnya terpisah — kegagalan eksekusi tidak pernah menggagalkan bot sinyal.
...
execution/okx_executor.py    # EKSEKUSI TERPISAH: OKX Futures via CCXT (sizing, SL/TP, hanya bila aktif)
...
- **ccxt** untuk eksekusi order di OKX Futures (USDT-M Perpetual) — hanya dibutuhkan saat autotrade aktif
...
- **SL/TP berbasis ATR** (`_atr_levels`): SL = entry ∓ `ATR_SL_MULT`×ATR(1H) (default 2.0x), TP1 = entry ± `ATR_TP1_MULT`×ATR, TP2 = entry ± `ATR_TP2_MULT`×ATR. SL juga dipasang **di luar swing low/high terdekat** (`support`/`resistance`) bila lebih aman, dibatasi `MAX_SL_MULT` agar R:R tetap sehat. Fallback persentase bila ATR kosong.
...
- Sinyal BUY/SELL **dipromosikan hanya jika ≥ `CONFLUENCE_MIN` item cek selaras** (default 2) **DAN** minimal `REQUIRE_CONFLUENCE_CORE` cek dari kategori inti (**S&D/S&R atau MACD/RSI**) selaras; selain itu diturunkan ke NEUTRAL (watchlist) dengan catatan alasan. SMC/OB + Whale/Vol saja **tidak cukup** — mencegah sinyal bermodal konfluensi sempit (mis. SMC/OB 2/2 tapi S&D/S&R 0/2 & MACD/RSI 0/2). Karena bot ini dapat **mengeksekusi order nyata di OKX Futures**, filter ini adalah pengaman utama terhadap sinyal lemah.
...
## 3. Automated Trading OKX (execution/okx_executor.py)

Modul **terpisah** dari mesin sinyal. Hanya berfungsi bila `ENABLE_OKX_AUTOTRADE=true`; bila false, `bot.py` melewati eksekusi tanpa efek samping.

### Variabel `.env` baru

| Variabel | Default | Keterangan |
|---|---|---|
| `OKX_API_KEY` | kosong | API Key OKX (https://www.okx.com/account/my-api, izin Trade) |
| `OKX_SECRET_KEY` | kosong | Secret Key OKX |
| `OKX_PASSPHRASE` | kosong | PASS PHRASE dibuat saat membuat API key (BUKAN sandi akun) |
| `ENABLE_OKX_AUTOTRADE` | `false` | Sakelar utama autotrade; `true` = order NYATA |
...
1. `build_exchange()` → `ccxt.okx` (options `defaultType=swap`; passphrase masuk ke field `password`) + `load_markets()`. Raise `OkxExecutionError` bila kredensial kosong.
2. `market_symbol("BTC")` → `BTC/USDT:USDT` (USDT-M Perpetual).
3. `fetch_free_balance_usdt()` → free USDT akun swap; raise bila 0.
4. **Position sizing dinamis**: `risk = balance × RISK_PERCENT_PER_TRADE / 100`; koin = `risk / |entry − SL|` (arah LONG/SHORT sama). Nilai posisi dibatasi ≤ saldo (margin 1x, cross) agar tidak ditolak; koin dikonversi ke kontrak (`contractSize` OKX < 1 koin, mis. BTC = 0.01).
5. **Market order entry** dengan `tdMode='cross'` (wajib untuk swap OKX) + `stopLossPrice` terpasang di order — ccxt.okx mengubahnya menjadi **algo SL market** (`attachAlgoOrds`), SL full position.
...
### Kontrak tes (tests/test_okx_executor.py)
- Gunakan `FakeExchange` (tanpa network): memverifikasi urutan 3 order, side buy/sell, `stopLossPrice` pada order entry, `tdMode='cross'` di semua order, amount TP = 50% entry, `reduceOnly: true`.
- Skenario gagal: saldo 0, simbol tidak tersedia, action NEUTRAL, kredensial kosong, order entry ditolak → semuanya `OkxExecutionError`.
...
### Mengubah perilaku eksekusi OKX
- Logika order ada di `execution/okx_executor.py` → `execute_signal`. Sizing: `position_quantity` + `risk_amount_usd`. Semua order swap memakai `tdMode='cross'` (konstanta `OKX_TD_MODE`); SL terpasang via params `stopLossPrice` (ccxt → algo SL market OKX).
...
- `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_ADMIN_CHAT_ID`, `OKX_API_KEY`, `OKX_SECRET_KEY`, `OKX_PASSPHRASE` disimpan di `.env` — sudah masuk `.gitignore`, tidak boleh di-commit.
...
- Di GitHub Actions, kredensial disuntikkan lewat secrets; `MTF_SCAN_LIMIT`/`CONFLUENCE_MIN`/`ENABLE_OKX_AUTOTRADE` opsional lewat `vars`.
- Saat debugging, jangan pernah mencetak isi token/secret ke log atau output.
- Untuk OKX: gunakan API key dengan izin minimal **Trade** (jangan pernah aktifkan Withdraw). Passphrase dibuat saat membuat key (8–32 karakter) dan masuk ke field `password` ccxt. Test di akun demo/posisi kecil sebelum nyalakan `ENABLE_OKX_AUTOTRADE=true`.
